// ============================================================================
// Set Availability - Intersection By Key Write Kernel (Strategy A / v1)
//
...
// - Compare keys only
// - When keys match, output A's key AND A's value (from first range)
...
fn device_load_keys_and_values_to_shared(
    ...
    // Load B's keys only (we don't need B's values for intersection)
    i = tid;
...
// ============================================================================
// Serial Set Intersection By Key (ModernGPU Style)
//
// For intersection: commit when A_key == B_key
// Always store A's index so we can output A's key and A's value
// ============================================================================
fn serial_set_intersection_by_key(
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        if (!extended) {
            if (a_idx >= a_end) {
                // A exhausted, advance B only
                b_idx++;
                continue;
            }
            if (b_idx >= b_end) {
                // B exhausted, advance A only
                a_idx++;
                continue;
            }
        }
        ...
        // Intersection: commit only when equal (!pA && !pB)
        // Store A's key, value, and index only when we commit
        if (!pA && !pB) {
            (*result_keys)[i] = a_key;
            (*result_values)[i] = a_values_shared[a_idx];  // A's value
            (*indices)[i] = a_idx;
            commit |= (1u << i);
        }
...
// ============================================================================
// Main Kernel: Write Intersection By Key
// ============================================================================
...
fn write_availability(
    ...
    // Load keys and A's values (B's values not needed for intersection)
