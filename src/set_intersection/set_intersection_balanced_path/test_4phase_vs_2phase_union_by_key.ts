/**
 * Test file for 4-Phase vs 2-Phase Union By Key Comparison
 *
 * Compares the 4-phase pipeline with the 2-phase sentinel pipeline
 * for union by key operations (compare keys, output key-value pairs).
 *
 * 4-Phase (baseline):
 *   VT=7, NV=1792
 *   1. DPI - Compute diagonal path indices
 *   2. Count - Count union output per workgroup (keys only)
 *   3. Prefix Sum - Exclusive scan for output offsets
 *   4. Write - Write union keys + values
 *
...
import countByKeyShader from './set_availability_union_by_key_count_v1.wgsl';
import writeByKeyShader from './set_availability_union_by_key_write_v1.wgsl';
import sentinelByKeyLookbackShader from './set_availability_union_by_key_decoupled_lookback_sentinel.wgsl';
...
class FourPhaseByKeyTester {
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.diagBindGroupLayout = device.createBindGroupLayout({
            label: '4P-UnionByKey DPI bind group layout',
            entries: [
        ...
        this.diagPipeline = device.createComputePipeline({
            label: '4P-UnionByKey DPI pipeline',
        ...
        this.countBindGroupLayout = device.createBindGroupLayout({
            label: '4P-UnionByKey Count bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
        ...
        this.countPipeline = device.createComputePipeline({
            label: '4P-UnionByKey Count pipeline',
        ...
        // Write: 11 bindings (a_keys, a_values, b_keys, b_values, dpi, offsets, output_keys, output_values, a_length, b_length, num_wg_total)
        this.writeBindGroupLayout = device.createBindGroupLayout({
            label: '4P-UnionByKey Write bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 2, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
        ...
        this.writePipeline = device.createComputePipeline({
            label: '4P-UnionByKey Write pipeline',
    ...
    public async run(
        ...
        // Union: maxOutputSize = a_len + b_len (worst case: no overlap)
...
class SentinelByKeyTester {
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.diagBindGroupLayout = device.createBindGroupLayout({
            label: 'Sentinel-UnionByKey DPI bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 2, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'storage' } },
        ...
        this.diagPipeline = device.createComputePipeline({
            label: 'Sentinel-UnionByKey DPI pipeline',
            layout: device.createPipelineLayout({ bindGroupLayouts: [this.diagBindGroupLayout] }),
            compute: {
        ...
        // Lookback: 12 bindings
        // a_keys(0), a_values(1), b_keys(2), b_values(3), dpi(4), state(5),
        // output_keys(6), output_values(7), total_count(8),
        // a_length(9), b_length(10), num_wg_total(11)
        this.lookbackBindGroupLayout = device.createBindGroupLayout({
            label: 'Sentinel-UnionByKey Lookback bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 2, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
        ...
        this.lookbackPipeline = device.createComputePipeline({
            label: 'Sentinel-UnionByKey Lookback pipeline',
            ...
            compute: {
                ...
                entryPoint: 'union_by_key_decoupled_lookback'
            }
        });
    }
...
function cpuSetUnionByKey(
    ...
    while (ai < aKeys.length && bi < bKeys.length) {
        if (aKeys[ai] < bKeys[bi]) {
            ...
            resultKeys.push(bKeys[bi]);
            resultValues.push(bValues[bi]);
            bi++;
        } else {
...
/**
 * Run 4-Phase vs 2-Phase Union By Key comparison on all datasets.
 */
export async function run4PhaseVs2PhaseUnionByKeyTest(device: GPUDevice): Promise<void> {
    console.log('\n' + '='.repeat(80));
    console.log('  UNION BY KEY: 4-PHASE vs 2-PHASE SENTINEL');
    ...
    // Correctness Tests
    console.log('-'.repeat(80));
    console.log('CORRECTNESS TESTS (Union By Key)');
    console.log('-'.repeat(80) + '\n');
    ...
    for (const test of syntheticTests) {
        ...
        const bValues = generateValues(test.bKeys);
        const cpuResult = cpuSetUnionByKey(test.aKeys, aValues, test.bKeys, bValues);
        const cpuCount = cpuResult.keys.length;

        const fourResult = await fourPhaseTester.run(test.aKeys, aValues, test.bKeys, bValues, 1, 0);
        const sentResult = await sentinelTester.run(test.aKeys, aValues, test.bKeys, bValues, 1, 0);
    ...
    console.log('-'.repeat(80));
    console.log('PERFORMANCE BENCHMARKS (Union By Key)');
    console.log('-'.repeat(80) + '\n');
    ...
    for (const { size, range } of datasets) {
        ...
        try {
            ...
            const sentResult = await sentinelTester.run(aKeys, aValues, bKeys, bValues, NUM_ITERATIONS, NUM_WARMUP);
    ...
    // Summary
    console.log('\n' + '='.repeat(80));
    console.log('  TOTAL TIME SUMMARY (Union By Key: 4-Phase vs 2-Phase Sentinel)');
    console.log('='.repeat(80));
    ...
    console.log('    By-Key: outputs keys+values from A or B');
    console.log('');
}

/**
 * Run 128M only comparison for union by key.
 */
export async function run4PhaseVs2PhaseUnionByKey128MTest(device: GPUDevice): Promise<void> {
    console.log('\n' + '='.repeat(80));
    console.log('  UNION BY KEY: 4-PHASE vs 2-PHASE SENTINEL (128M)');
    console.log('='.repeat(80) + '\n');
