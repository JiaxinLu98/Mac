// ============================================================================
// Set Availability - Intersection By Key with Decoupled Lookback - SENTINEL
//
...
// - Compare keys only
// - When keys match, output A's key AND A's value (Thrust semantics)
// - B's values are not needed
//
// Flow:
...
// 4. Each thread runs SerialSetIntersectionByKey to get results
...
@group(0) @binding(7) var<storage, read_write> total_count: array<atomic<u32>, 1>;
...
// ============================================================================
// keys_shared: A keys + B keys with sentinels (same as non-by-key sentinel)
...
// ============================================================================
// Serial Set Intersection By Key - with sentinel offset
// Branchless select() for pointer advancement + pre-fetch pattern.
// Physical indices are +1 from logical indices.
//
// When keys match, store A's key and A's value (Thrust semantics).
// ============================================================================
fn serial_set_intersection_by_key(
    a_begin: u32,
    a_end: u32,
    b_begin: u32,
    b_end: u32,
    ...
    // Pre-fetch first keys and value (physical = logical/physical + 1 for sentinel)
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        // Intersection: commit when equal (pA == pB, both false)
        if (pA == pB) {
            commit |= (1u << i);
        }
        ...
        // Pre-fetch next keys and value (with +1 physical offset)
...
fn intersection_by_key_decoupled_lookback(
