// ============================================================================
// Set Availability - Intersection with Decoupled Lookback - SENTINEL (VT=15)
//
// Single-pass approach using Decoupled Lookback for prefix sum computation.
// Combines Count + Scan + Write in one kernel.
//
// WARNING: VT=15 may exceed 16KB workgroup memory limit on some GPUs.
//
...
const VT: u32 = 15u;            // Values per thread
const NV: u32 = NT * VT;        // Total elements per workgroup = 3840
...
// ============================================================================
// Original layout (NV + VT + 2 u32):
//   keys_shared[0 .. a_count)                    = A data
//   keys_shared[b_start .. b_start+b_count)      = B data
//
// Sentinel layout (3861 u32):
...
//
// Total size: NV + VT + 2 (original) + 4 (sentinels) = 3861
var<workgroup> keys_shared: array<u32, 3861>;
...
fn device_load_2_to_shared(
    ...
    // Load A elements: unrolled 15 iterations, with +1 offset for sentinel
    if (tid < a_load_count) {
    ...
    if (tid + 3072u < a_load_count) {
        keys_shared[tid + 3073u] = a[a_global_offset + tid + 3072u];
    }
    if (tid + 3328u < a_load_count) {
        keys_shared[tid + 3329u] = a[a_global_offset + tid + 3328u];
    }
    if (tid + 3584u < a_load_count) {
        keys_shared[tid + 3585u] = a[a_global_offset + tid + 3584u];
    }

    // Load B elements: unrolled 15 iterations, with +1 offset for sentinel
    if (tid < b_load_count) {
    ...
    if (tid + 3072u < b_load_count) {
        keys_shared[b_shared_start + tid + 3073u] = b[b_global_offset + tid + 3072u];
    }
    if (tid + 3328u < b_load_count) {
        keys_shared[b_shared_start + tid + 3329u] = b[b_global_offset + tid + 3328u];
    }
    if (tid + 3584u < b_load_count) {
        keys_shared[b_shared_start + tid + 3585u] = b[b_global_offset + tid + 3584u];
    }
}
...
//
// Since VT=15 < 16, get_local_biased_levels(VT) always returns 0.
// Biased search levels are only useful for larger partition sizes.
...
fn lower_bound_local_a(end_exclusive: u32, key: u32) -> u32 {
    ...
    while (lo < hi) {
        let mid = (lo + hi) >> 1u;
        // Physical access: keys_shared[mid + 1]
...
fn lower_bound_local_b(b_start: u32, end_exclusive: u32, key: u32) -> u32 {
    ...
    while (lo < hi) {
        let mid = (lo + hi) >> 1u;
        // Physical access: keys_shared[b_start + mid + 1]
...
fn balanced_path_local(
    ...
    if (b_index < b_count) {
        ...
        // Simplified: no biased levels needed for VT=15 (< 16)
...
// ============================================================================
// Serial Set Intersection - with sentinel offset
...
fn serial_set_intersection(
    ...
    b_adjust: u32,
    extended: bool,
    results: ptr<function, array<u32, 15>>,
    indices: ptr<function, array<u32, 15>>
) -> u32 {
    var commit: u32 = 0u;
    var a_idx = a_begin;
    var b_idx = b_begin;
...
// Returns the exclusive prefix (sum of all previous workgroups)
// ============================================================================
fn decoupled_lookback(wg_id: u32, local_total: u32) -> u32 {
    var exclusive_prefix: u32 = 0u;

    // Workgroup 0 is special: no predecessors
    if (wg_id == 0u) {
        // Publish INCLUSIVE state immediately
        atomicStore(&state[0u], pack_state(STATUS_INCLUSIVE, local_total));
        return 0u;
    }

    // Other workgroups: publish PARTIAL first, then lookback
    atomicStore(&state[wg_id], pack_state(STATUS_PARTIAL, local_total));

    // Lookback loop: scan backwards to find global prefix
    var lookback_id: i32 = i32(wg_id) - 1;
    var running_sum: u32 = 0u;

    while (lookback_id >= 0) {
        // Spin-wait until predecessor has published state
        ...
        loop {
            ...
            // Spin: predecessor not ready yet
        }
        ...
        if (flag == STATUS_INCLUSIVE) {
            // Found an INCLUSIVE state: we're done
            running_sum += value;
            break;
        } else {
            // PARTIAL state: accumulate and continue looking back
            running_sum += value;
            lookback_id -= 1;
        }
    }
    ...
    // Update our state from PARTIAL to INCLUSIVE
...
fn intersection_decoupled_lookback(
    ...
    // BalancedPath returns logical indices (0-based into data, not counting sentinels)
    ...
    // ========================================================================
    var results: array<u32, 15>;
    var indices: array<u32, 15>;

    // For serial_set_intersection:
    // - a0tid, a_count2 are logical A indices
    // - b_start + b0tid is the physical B start (already includes b_start offset)
    ...
    // ========================================================================
    // Step 7: Scatter results to output - Unrolled (VT=15 iterations)
    // ========================================================================
    ...
    if ((commit & 0x1000u) != 0u) { output[write_pos] = results[12]; write_pos++; }
    if ((commit & 0x2000u) != 0u) { output[write_pos] = results[13]; write_pos++; }
    if ((commit & 0x4000u) != 0u) { output[write_pos] = results[14]; write_pos++; }
}
