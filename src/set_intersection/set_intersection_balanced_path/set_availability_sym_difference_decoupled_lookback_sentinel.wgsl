// ============================================================================
// Set Availability - Symmetric Difference with Decoupled Lookback - SENTINEL
//
...
// ============================================================================
// Serial Set Symmetric Difference - with sentinel offset
// Branchless select() for pointer advancement + pre-fetch pattern.
// Physical indices are +1 from logical indices.
//
// Emit when A != B (XOR condition)
// - pA (A < B): emit A, advance A (and B stays)
// - pB (B < A): emit B, advance B (and A stays)
...
fn serial_set_sym_difference(
    ...
    // Pre-fetch first keys (with +1 sentinel offset)
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        // Symmetric difference: emit the smaller key (or whichever is unique)
        (*results)[i] = select(b_key, a_key, pA);
        (*indices)[i] = select(b_idx, a_idx, pA);

        // Commit when exactly one of pA/pB is true (element in only one set)
        if (pA != pB) {
            commit |= (1u << i);
        }
        ...
        // Pre-fetch next keys (with +1 sentinel offset)
