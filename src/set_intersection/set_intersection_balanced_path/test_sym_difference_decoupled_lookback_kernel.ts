/**
 * Test file for Symmetric Difference Decoupled Lookback Kernel
 *
 * Tests the decoupled lookback implementation of set symmetric difference.
 * This kernel combines Count + Scan + Write in a single pass using
 * the decoupled lookback technique for streaming prefix sum.
 */

import { GPUSetSymDifferenceDecoupledLookback } from './set_sym_difference_decoupled_lookback';
import { TestSetSymDifferencePipeline } from './test_sym_difference_write_kernel';
import TimestampQueryManager from '../../TimestampQueryManager';
import * as utils from '../../utils';
import computeDiagonalsShader from './balanced_path_biased.wgsl';
import decoupledLookbackShader from './set_availability_sym_difference_decoupled_lookback.wgsl';
...
/**
 * Test class for the symmetric difference decoupled lookback kernel.
 */
export class TestSymDifferenceDecoupledLookbackKernel {
    private device: GPUDevice;
    private timestampQueryManager: TimestampQueryManager;
    private symDiff: GPUSetSymDifferenceDecoupledLookback;
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.symDiff = new GPUSetSymDifferenceDecoupledLookback(device, timestampQueryManager);
        ...
        this.lookbackBindGroupLayout = device.createBindGroupLayout({
            label: 'SymDiff Decoupled Lookback bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
        ...
        this.lookbackPipeline = device.createComputePipeline({
            label: 'SymDiff Decoupled Lookback pipeline',
            ...
            compute: {
                ...
                entryPoint: 'sym_difference_decoupled_lookback'
            }
        });
    }

    /**
     * Run the decoupled lookback symmetric difference test.
     */
    public async testDecoupledLookback(
        ...
        const result = await this.symDiff.computeSymDifference(setA, setB, 1);
        return {
    ...
    /**
     * CPU merge-path set symmetric difference.
    ...
    public cpuSetSymDifference(a: Uint32Array, b: Uint32Array): Uint32Array {
        ...
        while (ai < a.length && bi < b.length) {
            if (a[ai] < b[bi]) {
                ...
                // Equal: skip both (element in both sets)
                ai++;
                bi++;
            }
        }
    ...
    /**
     * CPU set symmetric difference count.
     */
    public cpuSetSymDifferenceCount(a: Uint32Array, b: Uint32Array): number {
        return this.cpuSetSymDifference(a, b).length;
    }
    ...
    public printResults(
        ...
        console.log('\n=== Symmetric Difference Decoupled Lookback Results ===');
    ...
    /**
     * Compute symmetric difference with GPU timestamp profiling.
     */
    public async computeSymDifferenceWithGPUProfiling(
        setA: Uint32Array,
        setB: Uint32Array,
        ...
        const maxOutputSize = a_len + b_len;  // SymDiff max output = a_len + b_len
...
/**
 * Run all symmetric difference decoupled lookback test cases.
 */
export async function runSymDifferenceDecoupledLookbackTest(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    SYM DIFFERENCE DECOUPLED LOOKBACK KERNEL TEST SUITE    ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');

    const timestampQueryManager = new TimestampQueryManager(device, 16);
    const tester = new TestSymDifferenceDecoupledLookbackKernel(device, timestampQueryManager);
    ...
    // ========================================================================
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('Test Case 1: Basic symmetric difference');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
        ...
        console.log('Expected: [1, 2, 4, 5, 8, 9, 10]');
    ...
    // ========================================================================
    // Test Case 2: No overlap (all elements in result)
    // ========================================================================
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('Test Case 2: No overlap (all elements in result)');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
        const A = new Uint32Array([1, 3, 5, 7, 9]);
    ...
    // ========================================================================
    // Test Case 4: Multiset - A has more copies than B
    // ========================================================================
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('Test Case 4: Multiset - A has more copies than B');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
        ...
        console.log('Expected:', Array.from(cpuResult));
    ...
    // ========================================================================
    // Test Case 5: Multiset - B has more copies than A
    // ========================================================================
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('Test Case 5: Multiset - B has more copies than A');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
        ...
        const cpuResult = tester.cpuSetSymDifference(A, B);
        console.log(`Expected: [42 x ${cpuResult.length}]`);
    ...
    console.log('╔════════════════════════════════════════════════════════════╗');
    if (allPassed) {
        console.log('║  ✔ ALL SYM DIFF LOOKBACK TESTS PASSED                    ║');
    } else {
        console.log('║  ✗ SOME SYM DIFF LOOKBACK TESTS FAILED                   ║');
    }
    console.log('╚════════════════════════════════════════════════════════════╝\n');
}

/**
 * Run GPU timestamp profiling benchmark for symmetric difference decoupled lookback.
 */
export async function runSymDifferenceDecoupledLookbackBenchmark(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    SYM DIFF DECOUPLED LOOKBACK GPU PROFILING              ║');
    ...
    console.log('╔══════════╤══════════════╤════════════════╤════════╤══════════╤════════╤════════╗');
    console.log('║ Dataset  │ Input Size   │ SymDiff Size   │ DPI(ms)│Lookbk(ms)│Total(ms)│ Valid  ║');
    console.log('╠══════════╪══════════════╪════════════════╪════════╪══════════╪════════╪════════╣');
    ...
    for (const { size, range } of datasets) {
        ...
        try {
            ...
            const { result, totalCount, timing } = await tester.computeSymDifferenceWithGPUProfiling(
            ...
            if (A.length <= 32_000_000) {
                const cpuCount = tester.cpuSetSymDifferenceCount(A, B);
            ...
            console.log(`║ ${ds} │ ${inputSize} │ ${symDiffSize} │ ${dpi} │ ${lookback} │ ${total} │${validStr}║`);
    ...
    console.log('Analysis hints (SymDiff Decoupled Lookback - 2 phases):');
...
/**
 * Back-to-back comparison: 4-Phase vs Decoupled Lookback for set symmetric difference.
 */
export async function runSymDifference4PhaseVsLookbackComparison(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    SYM DIFF: 4-PHASE vs LOOKBACK COMPARISON               ║');
    ...
    const fourPhase = new TestSetSymDifferencePipeline(device, timestampQueryManager);
    const lookback = new TestSymDifferenceDecoupledLookbackKernel(device, timestampQueryManager);
    ...
    for (const { size, range } of datasets) {
        ...
        try {
            ...
            // --- Run 4-Phase first ---
            const fp = await fourPhase.computeSetSymDifferenceWithGPUProfiling(A, B, NUM_ITERATIONS, NUM_WARMUP);

            // --- Run Lookback second ---
            const lb = await lookback.computeSymDifferenceWithGPUProfiling(A, B, NUM_ITERATIONS, NUM_WARMUP);
...
/**
 * Back-to-back comparison: 4-Phase vs Decoupled Lookback for set symmetric difference
 * on the 128M dataset only.
 */
export async function runSymDifferenceDecoupledLookback128MBenchmark(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    SYM DIFF: 4-PHASE vs LOOKBACK (128M ONLY)              ║');
    console.log('║    (Same run, same GPU state, pure GPU timestamps)        ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');
