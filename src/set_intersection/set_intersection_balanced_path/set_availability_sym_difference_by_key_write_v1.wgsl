// ============================================================================
// Set Availability - Symmetric Difference By Key Write Kernel (Strategy A / v1)
//
...
// - pA (A < B): emit A's key-value, advance A
// - pB (B < A): emit B's key-value, advance B
// - equal (!pA && !pB): advance both, no emit (one cancels one)
//
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        if (!extended) {
            if (a_idx >= a_end) {
                ...
                (*result_keys)[i] = keys_shared[b_idx];
        ...
        if (pA) {
            // A < B: emit A's key-value
            (*result_keys)[i] = a_key;
            (*result_values)[i] = a_values_shared[a_idx];
            commit |= (1u << i);
        } else if (pB) {
            // B < A: emit B's key-value
            (*result_keys)[i] = b_key;
            (*result_values)[i] = b_values[b_global_offset + (b_idx - b_start)];
            commit |= (1u << i);
        }
...
// ============================================================================
// Main Kernel: Write Symmetric Difference By Key
// ============================================================================
