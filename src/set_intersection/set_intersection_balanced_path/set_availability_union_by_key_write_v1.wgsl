// ============================================================================
// Set Availability - Union By Key Write Kernel (Strategy A / v1)
//
...
@group(0) @binding(8) var<uniform> a_length: u32;
@group(0) @binding(9) var<uniform> b_length: u32;
@group(0) @binding(10) var<uniform> num_wg_total: u32;
...
var<workgroup> a_values_shared: array<u32, 1801>;   // A values (B values read from global memory)
// NOTE: Only A values in shared memory to stay within 16KB limit
// keys_shared[1801] + a_values_shared[1801] + shared_scan[256] = 15,432 bytes < 16KB
...
fn device_load_keys_and_a_values_to_shared(
    tid: u32,
    a_global_offset: u32,
    a_load_count: u32,
    b_global_offset: u32,
    ...
    // Load A's keys and values to shared memory
    var i = tid;
    ...
    // Load B's keys only (B values read from global memory to save shared memory)
    i = tid;
...
// ============================================================================
// Serial Set Union By Key (ModernGPU Style)
//
// Emit every element from the merged sequence (multiset union)
// Compare keys only:
// - pA (A_key < B_key): emit A's key-value, advance A
// - pB (B_key < A_key): emit B's key-value, advance B
// - equal (!pA && !pB): emit A's key-value (tie goes to A), advance both
//
// NOTE: A values from shared memory, B values from global memory (to fit 16KB)
// ============================================================================
fn serial_set_union_by_key(
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        if (!extended && a_idx >= a_end) {
            ...
            // Both are in range - compare keys
            pA = a_key < b_key;
            pB = b_key < a_key;
        }

        // Output: if pB (B < A), output B's key-value; otherwise output A's key-value
        if (pB) {
            (*result_keys)[i] = b_key;
            // B values read from global memory
            // b_idx is in keys_shared coordinates (b_start + local_b_index)
            (*result_values)[i] = b_values[b_global_offset + (b_idx - b_start)];
        } else {
            (*result_keys)[i] = a_key;
            // A values read from shared memory
...
// ============================================================================
// Main Kernel: Write Union By Key
// ============================================================================
...
fn write_availability(
    ...
    // Load A keys+values and B keys to shared memory (B values read from global memory)
    device_load_keys_and_a_values_to_shared(tid, a0, a_load_count, b0, b_load_count, b_start);
    workgroupBarrier();
