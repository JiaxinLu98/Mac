/**
 * Test file for Union Decoupled Lookback Kernel
 *
 * Tests the decoupled lookback implementation of set union.
 * This kernel combines Count + Scan + Write in a single pass using
 * the decoupled lookback technique for streaming prefix sum.
 */

import { GPUSetUnionDecoupledLookback } from './set_union_decoupled_lookback';
import { TestSetUnionPipeline } from './test_union_write_kernel';
...
/**
 * Test class for the union decoupled lookback kernel.
 */
export class TestUnionDecoupledLookbackKernel {
    private device: GPUDevice;
    private timestampQueryManager: TimestampQueryManager;
    private union: GPUSetUnionDecoupledLookback;
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.union = new GPUSetUnionDecoupledLookback(device, timestampQueryManager);
    ...
    /**
     * Run the decoupled lookback union test.
     */
    public async testDecoupledLookback(
        ...
        const result = await this.union.computeUnion(setA, setB, 1);
        return {
    ...
    /**
     * CPU merge-path set union.
     * Returns the union elements (A U B).
     */
    ...
    public cpuSetUnionCount(a: Uint32Array, b: Uint32Array): number {
        ...
        let ai = 0, bi = 0;
        while (ai < a.length && bi < b.length) {
            if (a[ai] < b[bi]) { count++; ai++; }
            else if (a[ai] > b[bi]) { count++; bi++; }
            else { count++; ai++; bi++; }
        }
    ...
    public printResults(
        ...
        console.log('\n=== Union Decoupled Lookback Results ===');
    ...
    /**
     * Compute union with GPU timestamp profiling.
     */
    public async computeUnionWithGPUProfiling(
        setA: Uint32Array,
        setB: Uint32Array,
        ...
        const maxOutputSize = a_len + b_len;  // Union max output = a_len + b_len
...
/**
 * Run all union decoupled lookback test cases.
 */
export async function runUnionDecoupledLookbackTest(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    UNION DECOUPLED LOOKBACK KERNEL TEST SUITE             ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');

    const timestampQueryManager = new TimestampQueryManager(device, 16);
    const tester = new TestUnionDecoupledLookbackKernel(device, timestampQueryManager);

    let allPassed = true;

    // ========================================================================
    // Test Case 1: Basic multiset union
    // ========================================================================
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('Test Case 1: Basic multiset union');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
        ...
        console.log('Expected: [1, 2, 3, 4, 5, 7, 8, 9, 10]');
    ...
    // ========================================================================
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('Test Case 4: Multiset with duplicates (A has more copies)');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
        ...
        console.log('A:', Array.from(A));
        console.log('B:', Array.from(B));
        console.log('Expected: [5, 5, 5] (min copies = 3 matched, 2 unmatched from A → total output depends on union semantics)');
    ...
    // ========================================================================
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('Test Case 5: Multiset with duplicates (B has more copies)');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    {
    ...
    console.log('╔════════════════════════════════════════════════════════════╗');
    if (allPassed) {
        console.log('║  ✔ ALL UNION LOOKBACK TESTS PASSED                       ║');
    } else {
        console.log('║  ✗ SOME UNION LOOKBACK TESTS FAILED                      ║');
    }
    console.log('╚════════════════════════════════════════════════════════════╝\n');
}

/**
 * Run GPU timestamp profiling benchmark for union decoupled lookback.
 */
export async function runUnionDecoupledLookbackBenchmark(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    UNION DECOUPLED LOOKBACK GPU PROFILING                 ║');
    ...
    console.log('╔══════════╤══════════════╤════════════════╤════════╤══════════╤════════╤════════╗');
    console.log('║ Dataset  │ Input Size   │ Union Size     │ DPI(ms)│Lookbk(ms)│Total(ms)│ Valid  ║');
    console.log('╠══════════╪══════════════╪════════════════╪════════╪══════════╪════════╪════════╣');
    ...
    for (const { size, range } of datasets) {
        ...
        try {
            ...
            const { result, totalCount, timing } = await tester.computeUnionWithGPUProfiling(
            ...
            console.log(`║ ${ds} │ ${inputSize} │ ${unionSize} │ ${dpi} │ ${lookback} │ ${total} │${validStr}║`);
    ...
    console.log('Analysis hints (Union Decoupled Lookback - 2 phases):');
...
/**
 * Back-to-back comparison: 4-Phase vs Decoupled Lookback for set union.
 */
export async function runUnion4PhaseVsLookbackComparison(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    UNION: 4-PHASE vs LOOKBACK COMPARISON                  ║');
    ...
    const fourPhase = new TestSetUnionPipeline(device, timestampQueryManager);
    const lookback = new TestUnionDecoupledLookbackKernel(device, timestampQueryManager);
    ...
    for (const { size, range } of datasets) {
        ...
        try {
            ...
            // --- Run 4-Phase first ---
            const fp = await fourPhase.computeSetUnionWithGPUProfiling(A, B, NUM_ITERATIONS, NUM_WARMUP);

            // --- Run Lookback second ---
            const lb = await lookback.computeUnionWithGPUProfiling(A, B, NUM_ITERATIONS, NUM_WARMUP);
 ...
 */
export async function runUnion4PhaseVsLookback128MComparison(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║    UNION: 4-PHASE vs LOOKBACK (128M ONLY)                 ║');
    console.log('║    (Same run, same GPU state, pure GPU timestamps)        ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');
