// ============================================================================
// Set Availability - Difference with Decoupled Lookback - SENTINEL
//
...
// 4. Each thread runs SerialSetDifference to get results
...
//
// Total size: NV + VT + 2 (original) + 4 (sentinels) = 1805
var<workgroup> keys_shared: array<u32, 1805>;
...
fn device_load_2_to_shared(
    ...
    // Load A elements: unrolled 7 iterations, with +1 offset for sentinel
    if (tid < a_load_count) {
    ...
    // Load B elements: unrolled 7 iterations, with +1 offset for sentinel
    if (tid < b_load_count) {
...
//
// Since VT=7 < 16, get_local_biased_levels(VT) always returns 0.
...
fn balanced_path_local(
    ...
    if (b_index < b_count) {
        ...
        // Simplified: no biased levels needed for VT=7 (< 16)
...
// ============================================================================
// Serial Set Difference - with sentinel offset
// Branchless select() for pointer advancement + pre-fetch pattern.
// Physical indices are +1 from logical indices.
//
// With sentinels, boundary handling is automatic:
// - A exhausted → a_key = POS_INF → pA=false, pB=true → B advances, no emit
// - B exhausted → b_key = POS_INF → pA=true, pB=false → A emits, A advances
// ============================================================================
fn serial_set_difference(
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        // Difference: emit when pA is true (A < B, i.e., A not in B)
        if (pA) {
            commit |= (1u << i);
        }
...
fn difference_decoupled_lookback(
    ...
    // ========================================================================
    // Step 4: Serial set difference
    // ========================================================================
    var results: array<u32, 7>;
    var indices: array<u32, 7>;

    // For serial_set_difference:
    ...
    // ========================================================================
    // Step 7: Scatter results to output - Unrolled (VT=7 iterations)
    // ========================================================================
