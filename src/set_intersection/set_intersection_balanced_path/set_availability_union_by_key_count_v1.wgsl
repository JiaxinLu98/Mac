// ============================================================================
// Set Availability - Union By Key Count Kernel (Strategy A / v1)
//
...
// - Compare keys only
// - If A_key < B_key: emit from A, advance A
// - If B_key < A_key: emit from B, advance B
// - If A_key == B_key: emit from A (tie goes to A), advance both
// - Values are not needed in count phase
// ============================================================================
...
// - equal (!pA && !pB): emit from A (tie goes to A), advance both
// ============================================================================
fn serial_set_union_by_key(
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        // Output: if pB (B < A), output B's key; otherwise output A's key (includes tie case)
        if (pB) {
            (*results)[i] = b_key;
            (*indices)[i] = b_idx;
        } else {
...
// ============================================================================
// Main Kernel: Count Union By Key
// ============================================================================
