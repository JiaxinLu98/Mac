/**
 * GPUSetIntersectionBPMerge - Balanced Path Set Intersection with Merge-Based Count Phase
 *
 * This version uses count_intersections_merge.wgsl which leverages dual-pointer
 * merge traversal on A and B partitions, only performing global binary search
 * for values that appear in BOTH partitions.
 *
 * Key optimization: Instead of searching for every unique value in A's partition,
 ...
 *
 * Expected improvement: 2-10x speedup in count phase (higher speedup when fewer matches)
 */

import computeDiagonalsShader from './balanced_path.wgsl';
import countShaderMerge from './count_intersections_merge.wgsl';
...
export class GPUSetIntersectionBPMerge {
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        // ========== Count Intersections Pipeline (MERGE-BASED) ==========
        this.bindGroupLayoutCount = device.createBindGroupLayout({
            label: 'count intersections bind group layout (merge)',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } }, // a
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } }, // b
        ...
        const pipelineLayoutCount = device.createPipelineLayout({
            label: 'count intersections pipeline layout (merge)',
            bindGroupLayouts: [this.bindGroupLayoutCount]
        });

        // Use MERGE-BASED count shader
        const shader_count = device.createShaderModule({
            label: 'count intersections shader (merge-based)',
            code: countShaderMerge
        });

        this.countPipeline = device.createComputePipeline({
            label: 'count intersections pipeline (merge)',
    ...
    public async computeIntersection(
        ...
        if (partitionSize === undefined) {
            partitionSize = GPUSetIntersectionBPMerge.getOptimalPartitionSize(setA.length + setB.length, valueRange);
        }
        ...
        for (let i = 0; i < iters; i++) {
            ...
            // Phase 2: COUNT INTERSECTIONS (MERGE-BASED)
        ...
        console.log(
            `[Merge] ComputeIntersection x${iters}:`,
        ...
        console.log(
            `[Merge] GPU timestamps avg over ${iters} iterations:`,
    ...
    public async runCountPhase(
        ...
        const commandEncoder = device.createCommandEncoder({ label: 'Count Intersections (Merge)' });
        const base = this.getQueryBaseOffset();
    ...
    public validateCount(gpuTotalCount: number, setA: Uint32Array, setB: Uint32Array): boolean {
        ...
        if (valid) {
            console.log(`[Merge] Count validation PASSED: GPU=${gpuTotalCount}, CPU=${cpuCount}`);
        } else {
            console.log(`[Merge] Count validation FAILED: GPU=${gpuTotalCount}, CPU=${cpuCount}`);
        }
