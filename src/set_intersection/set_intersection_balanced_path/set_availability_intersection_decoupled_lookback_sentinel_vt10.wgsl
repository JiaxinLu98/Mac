...
const VT: u32 = 10u;            // Values per thread
const NV: u32 = NT * VT;        // Total elements per workgroup = 2560
...
//
// Total size: NV + VT + 2 (original) + 4 (sentinels) = 2576
var<workgroup> keys_shared: array<u32, 2576>;
...
fn device_load_2_to_shared(
    ...
    // Load A elements: unrolled 10 iterations, with +1 offset for sentinel
    if (tid < a_load_count) {
    ...
    // Load B elements: unrolled 10 iterations, with +1 offset for sentinel
    if (tid < b_load_count) {
...
//
// Since VT=10 < 16, get_local_biased_levels(VT) always returns 0.
...
fn balanced_path_local(
    ...
    if (b_index < b_count) {
        ...
        // Simplified: no biased levels needed for VT=10 (< 16)
...
fn serial_set_intersection(
    ...
    b_adjust: u32,
    extended: bool,
    results: ptr<function, array<u32, 10>>,
    indices: ptr<function, array<u32, 10>>
) -> u32 {
    var commit: u32 = 0u;
    var a_idx = a_begin;
    var b_idx = b_begin;
...
fn intersection_decoupled_lookback(
    ...
    // ========================================================================
    // Step 7: Scatter results to output - Unrolled (VT=10 iterations)
    // ========================================================================
