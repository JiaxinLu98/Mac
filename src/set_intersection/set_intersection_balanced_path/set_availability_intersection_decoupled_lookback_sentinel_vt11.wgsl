...
const VT: u32 = 11u;            // Values per thread
const NV: u32 = NT * VT;        // Total elements per workgroup = 2816
...
//
// Total size: NV + VT + 2 (original) + 4 (sentinels) = 2833
var<workgroup> keys_shared: array<u32, 2833>;
...
fn device_load_2_to_shared(
    ...
    // Load A elements: unrolled 11 iterations, with +1 offset for sentinel
    if (tid < a_load_count) {
    ...
    // Load B elements: unrolled 11 iterations, with +1 offset for sentinel
    if (tid < b_load_count) {
...
//
// Since VT=11 < 16, get_local_biased_levels(VT) always returns 0.
...
fn balanced_path_local(
    ...
    if (b_index < b_count) {
        ...
        // Simplified: no biased levels needed for VT=11 (< 16)
...
fn serial_set_intersection(
    ...
    b_adjust: u32,
    extended: bool,
    results: ptr<function, array<u32, 11>>,
    indices: ptr<function, array<u32, 11>>
) -> u32 {
    var commit: u32 = 0u;
    var a_idx = a_begin;
    var b_idx = b_begin;
...
fn intersection_decoupled_lookback(
    ...
    // ========================================================================
    var results: array<u32, 11>;
    var indices: array<u32, 11>;
    ...
    // ========================================================================
    // Step 7: Scatter results to output - Unrolled (VT=11 iterations)
    // ========================================================================
