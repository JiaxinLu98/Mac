/**
 * Test file for 4-Phase vs 2-Phase Intersection By Key Comparison
 *
 * Compares the 4-phase pipeline with the 2-phase sentinel pipeline
 * for intersection by key operations (compare keys, output key-value pairs from A).
 *
 * 4-Phase (baseline):
 *   VT=7, NV=1792
 *   1. DPI - Compute diagonal path indices
 *   2. Count - Count matches per workgroup (keys only)
 *   3. Prefix Sum - Exclusive scan for output offsets
 *   4. Write - Write intersection keys + values
 *
...
import countByKeyShader from './set_availability_intersection_by_key_count_v1.wgsl';
import writeByKeyShader from './set_availability_intersection_by_key_write_v1.wgsl';
import sentinelByKeyLookbackShader from './set_availability_intersection_by_key_decoupled_lookback_sentinel.wgsl';
...
class FourPhaseByKeyTester {
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.diagBindGroupLayout = device.createBindGroupLayout({
            label: '4P-ByKey DPI bind group layout',
            entries: [
        ...
        this.countBindGroupLayout = device.createBindGroupLayout({
            label: '4P-ByKey Count bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
        ...
        this.writeBindGroupLayout = device.createBindGroupLayout({
            label: '4P-ByKey Write bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 2, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
...
class SentinelByKeyTester {
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.diagBindGroupLayout = device.createBindGroupLayout({
            label: 'Sentinel-ByKey DPI bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 2, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'storage' } },
        ...
        this.diagPipeline = device.createComputePipeline({
            label: 'Sentinel-ByKey DPI pipeline',
            layout: device.createPipelineLayout({ bindGroupLayouts: [this.diagBindGroupLayout] }),
            compute: {
        ...
        this.lookbackBindGroupLayout = device.createBindGroupLayout({
            label: 'Sentinel-ByKey Lookback bind group layout',
            entries: [
                { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 1, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
                { binding: 2, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } },
        ...
        this.lookbackPipeline = device.createComputePipeline({
            label: 'Sentinel-ByKey Lookback pipeline',
            ...
            compute: {
                ...
                entryPoint: 'intersection_by_key_decoupled_lookback'
            }
        });
    }
...
function cpuSetIntersectionByKey(
    aKeys: Uint32Array,
    aValues: Uint32Array,
    bKeys: Uint32Array
...
/**
 * Run 4-Phase vs 2-Phase Intersection By Key comparison on all datasets.
 */
export async function run4PhaseVs2PhaseIntersectionByKeyTest(device: GPUDevice): Promise<void> {
    console.log('\n' + '='.repeat(80));
    console.log('  INTERSECTION BY KEY: 4-PHASE vs 2-PHASE SENTINEL');
    ...
    // Correctness Tests
    console.log('-'.repeat(80));
    console.log('CORRECTNESS TESTS (Intersection By Key)');
    console.log('-'.repeat(80) + '\n');
    ...
    for (const test of syntheticTests) {
        ...
        const cpuResult = cpuSetIntersectionByKey(test.aKeys, aValues, test.bKeys);
        ...
        // Also verify keys and values match
    ...
    console.log('-'.repeat(80));
    console.log('PERFORMANCE BENCHMARKS (Intersection By Key)');
    console.log('-'.repeat(80) + '\n');
    ...
    // Summary
    console.log('\n' + '='.repeat(80));
    console.log('  TOTAL TIME SUMMARY (Intersection By Key: 4-Phase vs 2-Phase Sentinel)');
    console.log('='.repeat(80));
...
/**
 * Run 128M only comparison for intersection by key.
 */
export async function run4PhaseVs2PhaseIntersectionByKey128MTest(device: GPUDevice): Promise<void> {
    console.log('\n' + '='.repeat(80));
    console.log('  INTERSECTION BY KEY: 4-PHASE vs 2-PHASE SENTINEL (128M)');
    console.log('='.repeat(80) + '\n');
