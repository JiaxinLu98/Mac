// ============================================================================
// Set Availability - Union Write Kernel (Strategy A / v1)
//
...
// ============================================================================
// Serial Set Union (ModernGPU Style)
//
// Emit every element from the merged sequence (multiset union)
// - pA (A < B): emit A, advance A
// - pB (B < A): emit B, advance B
// - equal (!pA && !pB): emit A (tie goes to A), advance both
// ============================================================================
fn serial_set_union(
    a_begin: u32,
    a_end: u32,
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        // Output: if pB (B < A), output B; otherwise output A (includes tie case)
        if (pB) {
            (*results)[i] = b_key;
            (*indices)[i] = b_idx;
        } else {
...
// ============================================================================
// Main Kernel: Write Union Results
// ============================================================================
