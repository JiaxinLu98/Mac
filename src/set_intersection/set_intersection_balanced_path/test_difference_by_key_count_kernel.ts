/**
 * Test file for Difference By Key Count Kernel (set_availability_difference_by_key_count_v1.wgsl)
 *
 * Tests the count phase of the ModernGPU-style difference by key algorithm.
 *
 * Thrust semantics for set_difference_by_key:
 * - Compare keys only
 * - Count elements in A whose keys are not fully covered by B
 * - If A has m copies of key x and B has n copies, output last max(m-n, 0) from A
 *
 * Multiset difference logic:
 * - pA (A_key < B_key): emit A, advance A (A not in B)
...
/**
 * Test class for the difference by key count kernel.
 */
export class TestDifferenceByKeyCountKernel {
    ...
    /**
     * CPU reference: difference by key count (Thrust semantics).
     * Returns total count of keys in A that are not fully covered by B.
     * If A has m copies of key x and B has n copies, contributes max(m-n, 0) to count.
     * Note: Only keys are compared, values are ignored for counting.
     */
    public cpuDifferenceByKeyCount(keysA: Uint32Array, keysB: Uint32Array): number {
        let count = 0;
        let ai = 0, bi = 0;
        while (ai < keysA.length && bi < keysB.length) {
            if (keysA[ai] < keysB[bi]) {
                // A_key < B_key: A is not in B, emit
                count++;
                ai++;
        ...
        count += (keysA.length - ai);
        return count;
    }

    /**
     * CPU difference by key - returns the actual result arrays.
     * Implements Thrust semantics: outputs last max(m-n, 0) key-value pairs from A.
     */
    public cpuDifferenceByKey(
        keysA: Uint32Array,
        valuesA: Uint32Array,
        keysB: Uint32Array
    ...
    public validateCount(gpuCount: number, keysA: Uint32Array, keysB: Uint32Array): boolean {
        const cpuCount = this.cpuDifferenceByKeyCount(keysA, keysB);
...
/**
 * Run difference by key count kernel tests.
 */
export async function runDifferenceByKeyCountKernelTest(device: GPUDevice): Promise<void> {
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║     DIFFERENCE BY KEY COUNT KERNEL TEST                    ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');

    const timestampQueryManager = new TimestampQueryManager(device, 16);
    const tester = new TestDifferenceByKeyCountKernel(device, timestampQueryManager);
    ...
    const testCases = [
        ...
        },
        {
            ...
            // All keys match => count = 0
        },
        {
            name: 'Multiset: A has more duplicates than B',
            keysA: new Uint32Array([1, 1, 1, 1, 1, 2, 2, 3]),
            valuesA: new Uint32Array([10, 11, 12, 13, 14, 20, 21, 30]),
            keysB: new Uint32Array([1, 1, 2, 3, 3]),
            valuesB: new Uint32Array([100, 101, 200, 300, 301]),
            // Key 1: 5 in A, 2 in B -> 3 remain (last 3: indices 2,3,4)
            // Key 2: 2 in A, 1 in B -> 1 remain (last 1: index 6)
            // Key 3: 1 in A, 2 in B -> 0 remain
            // Total: 3 + 1 + 0 = 4
        },
        {
            name: 'Multiset: B has more duplicates than A',
            keysA: new Uint32Array([1, 1, 2]),
            valuesA: new Uint32Array([10, 11, 20]),
            keysB: new Uint32Array([1, 1, 1, 1, 2, 2, 2]),
            valuesB: new Uint32Array([100, 101, 102, 103, 200, 201, 202]),
            // Key 1: 2 in A, 4 in B -> 0 remain
            // Key 2: 1 in A, 3 in B -> 0 remain
            // Total: 0
        },
        {
            name: 'B empty (all A remains)',
            keysA: new Uint32Array([1, 2, 3, 4, 5]),
            valuesA: new Uint32Array([10, 20, 30, 40, 50]),
            keysB: new Uint32Array([]),
            valuesB: new Uint32Array([]),
            // B empty => count = 5 (all A)
        },
        {
        ...
        {
            name: 'All same key with different multiplicities',
            keysA: new Uint32Array(100).fill(42),
            valuesA: new Uint32Array(100).map((_, i) => i),
            keysB: new Uint32Array(60).fill(42),
            valuesB: new Uint32Array(60).map((_, i) => i + 1000),
            // Key 42: 100 in A, 60 in B -> 40 remain (last 40 from A)
        },
    ];
    ...
    for (const { name, keysA, valuesA, keysB, valuesB } of testCases) {
        ...
        const cpuCount = tester.cpuDifferenceByKeyCount(keysA, keysB);
    ...
    console.log('\n' + '═'.repeat(60));
    if (allPassed) {
        console.log('✔ ALL DIFFERENCE BY KEY COUNT TESTS PASSED');
    } else {
