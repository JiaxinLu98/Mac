/**
 * Set Symmetric Difference using Balanced Path with Decoupled Lookback
 *
...
export class GPUSetSymDifferenceDecoupledLookback {
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.diagBindGroupLayout = device.createBindGroupLayout({
            label: 'SymDiff DPI bind group layout',
            entries: [
    ...
    public async computeSymDifference(
        setA: Uint32Array,
        setB: Uint32Array,
        iters: number = 1
    ): Promise<{
        ...
        const numWg = GPUSetSymDifferenceDecoupledLookback.calculateNumWorkgroups(a_len, b_len);
        this.computeQueries(a_len, b_len);
        const QUERIES_PER_ITER = this.queriesPerIter;

        // Symmetric difference max output = a_len + b_len (no overlap case)
        ...
        for (let i = 0; i < iters; i++) {
            ...
            const encoder = device.createCommandEncoder({ label: 'Decoupled Lookback SymDiff' });
        ...
        console.log(`[SymDiffDecoupledLookback] ComputeSymDiff x${iters}: wall total = ${(wallTotalMs / 1000).toFixed(9)} s, wall avg = ${(wallAvgMs / 1000).toFixed(9)} s/iter`);
        ...
        console.log(`[SymDiffDecoupledLookback] GPU timestamps avg over ${iters} iterations: dpi = ${avgDpiMs.toFixed(4)} ms, lookback = ${avgLookbackMs.toFixed(4)} ms, total = ${avgTotalMs.toFixed(4)} ms`);
    ...
    public cpuCountSymDifference(setA: Uint32Array, setB: Uint32Array): number {
        let count = 0;
        let ai = 0, bi = 0;
    ...
    public validateCount(gpuTotalCount: number, setA: Uint32Array, setB: Uint32Array): boolean {
        const cpuCount = this.cpuCountSymDifference(setA, setB);
        const valid = gpuTotalCount === cpuCount;
        if (valid) {
            console.log(`[SymDiffDecoupledLookback] Count validation PASSED: GPU=${gpuTotalCount}, CPU=${cpuCount}`);
        } else {
            console.log(`[SymDiffDecoupledLookback] Count validation FAILED: GPU=${gpuTotalCount}, CPU=${cpuCount}`);
        }
        return valid;
    }
}
