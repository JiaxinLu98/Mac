/**
 * GPUSetIntersectionBP_Optimized_V3 - Simplified optimization with hint-based search
 *
...
import writeShaderV3 from './write_intersections_optimized_v3.wgsl';  // V3 OPTIMIZED
...
export class GPUSetIntersectionBP_Optimized_V3 {
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        // Use V3 OPTIMIZED shader
        const shader_write = device.createShaderModule({
            label: 'write intersections shader (v3)',
            code: writeShaderV3
        });
    ...
    public async computeIntersection(
        ...
        if (partitionSize === undefined) {
            partitionSize = GPUSetIntersectionBP_Optimized_V3.getOptimalPartitionSize(setA.length + setB.length, valueRange);
        }
        ...
        console.log(
            `GPU timestamps (V3) avg over ${iters} iterations:`,
