...
//
// DISPATCH NOTE: The TypeScript caller must dispatch
...
fn compute_diagonals(@builtin(global_invocation_id) global_id: vec3<u32>,
    ...
    // For out-of-range k, clamp to diagonal 1 so all lanes still participate
    // in subgroup ops (WGSL requires subgroup-uniform control flow).
    // Only the final write is guarded by valid_k.
    let valid_k = (k > 0u && k < num_wg);
    ...
    // Only lane 0 of valid subgroups writes the result
