// ============================================================================
// Set Availability - Symmetric Difference By Key with Decoupled Lookback - SENTINEL
//
...
//
// For symmetric difference by key ((A \ B) U (B \ A)):
// - Compare keys only
// - When A_key < B_key: output A's key AND A's value, advance A
// - When B_key < A_key: output B's key AND B's value, advance B
// - When A_key == B_key: advance both, no output (one cancels one)
//
// A values from shared memory, B values from global memory (to fit 16KB limit)
//
// Flow:
...
// 4. Each thread runs SerialSetSymDifferenceByKey to get results
...
// ============================================================================
// keys_shared: A keys + B keys with sentinels (same layout as intersection by key sentinel)
...
// ============================================================================
// DeviceLoad: Cooperative loading with sentinels for keys and values
// Keys: offset by +1 with NEG_INF/POS_INF sentinels
// Values: offset by +1 (no sentinels, just matching layout)
// ============================================================================
...
// ============================================================================
// Serial Set Symmetric Difference By Key - with sentinel offset
// Branchless select() for pointer advancement + pre-fetch pattern.
// Physical indices are +1 from logical indices.
//
// Emit when A != B (XOR condition):
// - pA (A < B): emit A's key and A's value (from shared memory)
// - pB (B < A): emit B's key and B's value (from global memory)
// - equal (!pA && !pB): advance both, no emit (one cancels one)
//
// With sentinels, when one array is exhausted:
// - A exhausted: a_key = POS_INF > any b_key -> pB = true -> emit B
// - B exhausted: b_key = POS_INF > any a_key -> pA = true -> emit A
// ============================================================================
fn serial_set_sym_difference_by_key(
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        if (extended) {
            ...
            // Sentinels handle exhaustion: POS_INF naturally produces correct pA/pB
        ...
        // Symmetric difference by key: emit the unique element's key-value pair
        // Key selection is branchless
        (*result_keys)[i] = select(b_key, a_key, pA);
        (*indices)[i] = select(b_idx, a_idx, pA);

        // Value: A from shared memory, B from global memory
        if (pA) {
            (*result_values)[i] = a_val;
        } else {
            // pB: emit B's value from global memory
            // equal: value doesn't matter (won't be committed)
            (*result_values)[i] = b_values[b_global_offset + (b_idx - b_shared_start)];
        }

        // Commit when exactly one of pA/pB is true (keys not equal)
        if (pA != pB) {
            commit |= (1u << i);
        }
...
fn sym_difference_by_key_decoupled_lookback(
    ...
    // ========================================================================
    // Step 4: Serial set symmetric difference by key
    // ========================================================================
