...
const QUERIES_PER_ITER_GLOBAL = 22;
const SORT_SLOTS = 8;
const INTERSECTION_OFFSET = SORT_SLOTS * 2;

export class GPUSetIntersectionUnsorted {
    ...
    public computeQueries(lenA: number) {
        ...
        const maxScanSize = scan.maxScanSize;  // 512 * 512
    ...
    public async computeIntersection(
        ...
        for(let i = 0; i < iters; i++) {
            ...
            const baseIter = i * QUERIES_PER_ITER;

            // API CALL: SORT
            await this.runSortPhase(bufferA, lenA, baseIter + 0);
            await this.runSortPhase(bufferB, lenB, baseIter + 8);
            // API CALL: MARK
            const recordedFlags = await this.runMarkPhase(bufferA, bufferB, bufferFlags, setA.length, setB.length);
            // API CALL: SCAN
            await this.runExclusivePhase(bufferFlags, setA.length);
            // API CALL: SCATTER
            set_difference_result = await this.runScatterPhase(bufferA, setA.length, bufferFlags, recordedFlags);
        ...
        for (let i = 0; i < iters; ++i) {
            ...
            const baseSortA = base + 0;
            const zeroTicksA = timestamps[baseSortA + 1] - timestamps[baseSortA + 0];
            const histTicksA = timestamps[baseSortA + 3] - timestamps[baseSortA + 2];
            const prefixTicksA = timestamps[baseSortA + 5] - timestamps[baseSortA + 4];
            const scatterTicksA = timestamps[baseSortA + 7] - timestamps[baseSortA + 6];
            sumSortA += zeroTicksA + histTicksA + prefixTicksA + scatterTicksA;

            const baseSortB = base + 8;
            const zeroTicksB = timestamps[baseSortB + 1] - timestamps[baseSortB + 0];
            const histTicksB = timestamps[baseSortB + 3] - timestamps[baseSortB + 2];
            const prefixTicksB = timestamps[baseSortB + 5] - timestamps[baseSortB + 4];
            const scatterTicksB = timestamps[baseSortB + 7] - timestamps[baseSortB + 6];
            sumSortB += zeroTicksB + histTicksB + prefixTicksB + scatterTicksB;

            // mark
            const baseIntersection = base + INTERSECTION_OFFSET;
            const markTicks = timestamps[baseIntersection + 1] - timestamps[baseIntersection + 0];

            // scan
            const scanFirstChunkTicks = timestamps[baseIntersection + 3] - timestamps[baseIntersection + 2];
            const scanTicksApprox = scanFirstChunkTicks * this.numScanChunks;

            // scatter
            const scatterTicks = timestamps[baseIntersection + 5] - timestamps[baseIntersection + 4];
        ...
        const avgSeconds = {
            sortA:   avgTicks.sortA   * timestampPeriod,
        ...
        console.log(
            ...
            `sortA = ${avgSeconds.sortA.toFixed(9)} s,`,
            ...
            `total = ${(avgSeconds.sortA + avgSeconds.sortB + avgSeconds.mark + avgSeconds.scan + avgSeconds.scatter).toFixed(9)} s`
        );
    ...
    private async runSortPhase(
        ...
        initEncoder.copyBufferToBuffer(dataBuffer, 0, sortBuffer.keysA, 0, length * 4);
        ...
        zeroPayload.unmap();
        initEncoder.copyBufferToBuffer(zeroPayload, 0, sortBuffer.payloadA, 0, length * 4);
        device.queue.submit([initEncoder.finish()]);

        await sorter.sort(device.queue, sortBuffer, length, baseIndex);

        const outEncoder = device.createCommandEncoder();
        outEncoder.copyBufferToBuffer(sortBuffer.keysA, 0, dataBuffer, 0, length * 4);
    ...
    public async runScatterPhase(bufferASet: GPUBuffer, lenA: number, bufferFlags: GPUBuffer, recordedFlags: GPUBuffer) {
        ...
        const commandEncoder = this.device.createCommandEncoder({label: "Scatter"});
        ...
        size:  result_size * Uint32Array.BYTES_PER_ELEMENT,
        usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ
        });
        commandEncoder1.copyBufferToBuffer(bufferResult, 0, readbackBuffer, 0, result_size * Uint32Array.BYTES_PER_ELEMENT);
