  ...
  uniformBufferHashSize: GPUBuffer;
	...
	public async buildHashTable(
		...
		const bufferHashTable = device.createBuffer({
			...
			size: hash_table_size * Uint32Array.BYTES_PER_ELEMENT,
			usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST
		});
		const hashInit = new Uint32Array(hash_table_size).fill(0xffffffff);
    ...
    const aInUsedBuffer = device.createBuffer({
      label: 'hash table used flags',
      size: hash_table_size * Uint32Array.BYTES_PER_ELEMENT,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC,
    });
    const usedInit = new Uint32Array(hash_table_size).fill(0);
    device.queue.writeBuffer(aInUsedBuffer, 0, usedInit);
    ...
    const groupY = Math.ceil(workGroup / groupX);
		...
		const commandEncoder = device.createCommandEncoder({label:"Build"});
		...
		// Return results.
		return {bufferHashTable, bufferHashTableSize, aInUsedBuffer};
	}
	...
	) 
	{
		...
		entries: [
			...
			{binding: 6, resource: {buffer: aInUsedBuffer}},
		]
		});
    ...
    const commandEncoder = device.createCommandEncoder({label:"Join"});
		const base = this.getQueryBaseOffset();
	...
	public async runSortPhase(
		dataBuffer: GPUBuffer,
		bufferResultSize: GPUBuffer
	) {
		const device = this.device;

		const numKeys = await readSize(device, bufferResultSize);
		const sorter = new GPUSorter(device, 16, this.timestampQueryManager);
		const sortBuffer = sorter.createSortBuffers(numKeys);

		const initEncoder = device.createCommandEncoder();
    initEncoder.copyBufferToBuffer(dataBuffer, 0, sortBuffer.keysA, 0, numKeys * 4);
    ...
    zeroPayload.unmap();
    initEncoder.copyBufferToBuffer(zeroPayload, 0, sortBuffer.payloadA, 0, numKeys * 4);
    device.queue.submit([initEncoder.finish()]);

		await sorter.sort(device.queue, sortBuffer, numKeys);

		const outEncoder = device.createCommandEncoder();
    outEncoder.copyBufferToBuffer(sortBuffer.keysA, 0, dataBuffer, 0, numKeys * 4);
