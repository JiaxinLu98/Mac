...
@group(0) @binding(0) var<storage, read_write> a_in: array<u32>;
@group(0) @binding(1) var<uniform> a_in_size: u32;
@group(0) @binding(2) var<storage, read_write> a_in_hash_table: array<atomic<u32>>;
...
fn build_hash_table(@builtin(global_invocation_id) global_id: vec3<u32>, 
    ...
    for(var i = begin; i < end; i = i + 1u) {
        ...
        loop {
            let existing_key = atomicCompareExchangeWeak(&a_in_hash_table[position], EMPTY_KEY, key);
