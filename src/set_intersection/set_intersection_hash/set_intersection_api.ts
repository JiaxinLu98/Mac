...
export async function setIntersection(
  device: GPUDevice,
  aData: Uint32Array,
  bData: Uint32Array,
  iters: number,
  ...
  const maxResultSize = Math.min(a_in.length, b_in.length);
  const bufferResult = device.createBuffer({
    label: 'intersection result buffer',
    size:  maxResultSize * Uint32Array.BYTES_PER_ELEMENT,
  ...
  for(let i = 0; i < iters; i++) {
    ...
    const { bufferHashTable, bufferHashTableSize, aInUsedBuffer } = await setIntersection.buildHashTable(a_in.length, bufferA);
    // API CALL: HASH JOIN
    await setIntersection.getJoinResult(bufferHashTable, bufferHashTableSize, bufferB, bufferBSize, bufferResult, bufferResultSize, aInUsedBuffer, b_in.length);
    // API CALL: SORT
    await setIntersection.runSortPhase(bufferResult, bufferResultSize);
  ...
  console.log(
    `Wall-clock over ${iters} iterations: ` + `avg = ${wallClockAvgS.toFixed(9)} s/iter`);
  ...
  commandEncoder.copyBufferToBuffer(bufferResult, 0, readbackBuffer, 0, resultSize * Uint32Array.BYTES_PER_ELEMENT);
