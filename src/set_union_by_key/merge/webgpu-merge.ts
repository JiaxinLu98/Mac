import merge_path from './merge_path.wgsl';
import TimestampQueryManager from '../../TimestampQueryManager';

export class WebGPUMerger {
    PADDING: number = 1024;
    ...
    timesOfBinarySearchBuffer: GPUBuffer;
    timesOfBinarySearchBuffer1: GPUBuffer;
    countsOfThreadLoopBuffer: GPUBuffer;
    ...
    constructor(device: GPUDevice, timestampQueryManager: TimestampQueryManager) {
        ...
        this.timesOfBinarySearchBuffer = device.createBuffer({
            label: 'Times of Binary Search',
            size: this.NUM_WORKGROUPS * 4,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
        });

        this.timesOfBinarySearchBuffer1 = device.createBuffer({
            label: 'Times of Binary Search 1',
            size: 4,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
        });

        this.countsOfThreadLoopBuffer = device.createBuffer({
            label: 'Counts of while loop',
            size: 4 * 8 * 128,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
        });
    }

    public async merge(a_in: Uint32Array, b_in: Uint32Array, iters: number) {
        const device = this.device;
        const QUERIES_PER_ITER = 4; 

        // Inputs are expected to be pre-sorted and flattened arrays.
        const a_sorted = new Uint32Array(a_in);
        const b_sorted = new Uint32Array(b_in);

        const a_len = a_sorted.length / 2;
        const b_len = b_sorted.length / 2;
        ...
        const a_padded_data = new Uint32Array(a_padded_len * 2);
        a_padded_data.set(a_sorted);
        for (let i = a_sorted.length; i < a_padded_data.length; i++) {
            a_padded_data[i] = POSITIVE_INFINITY;
        }

        const b_padded_data = new Uint32Array(b_padded_len * 2);
        b_padded_data.set(b_sorted);
        for (let i = b_sorted.length; i < b_padded_data.length; i++) {
        ...
        const bufferC = device.createBuffer({
            ...
            size: c_len * 2 * Uint32Array.BYTES_PER_ELEMENT,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
        });
        ...
        device.queue.writeBuffer(this.timesOfBinarySearchBuffer, 0, new Uint32Array([0]));
        device.queue.writeBuffer(this.timesOfBinarySearchBuffer1, 0, new Uint32Array([0]));
        device.queue.writeBuffer(this.countsOfThreadLoopBuffer, 0, new Uint32Array([0]));
        ...
        const bindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                { binding: 7, resource: { buffer: this.timesOfBinarySearchBuffer } },
                { binding: 8, resource: { buffer: this.timesOfBinarySearchBuffer1 } },
                { binding: 9, resource: { buffer: this.countsOfThreadLoopBuffer } }
            ],
        });
