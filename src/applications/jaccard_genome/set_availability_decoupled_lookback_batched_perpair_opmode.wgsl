// ============================================================================
// Unified Set Availability - Decoupled Lookback - BATCHED PER-PAIR COUNT (OP_MODE)
//
// Supports all 4 set operations via OP_MODE (replaceable by setOpMode()).
// Used by batched Jaccard similarity: run once with OP_MODE=0 (intersection),
// once with OP_MODE=2 (union), share DPI results.
// ============================================================================
...
@group(0) @binding(5) var<storage, read> wgInfo: array<u32>;
@group(0) @binding(6) var<storage, read> pairIdPerWg: array<u32>;
@group(0) @binding(7) var<uniform> totalWg_uniform: u32;
...
const WG_INFO_STRIDE: u32 = 8u;  // same as batched DPI shader
...
// Per-pair info loaded from wgInfo
...
// ============================================================================
// Serial Set Operation - count only, respects OP_MODE
//   0 = intersection, 1 = difference, 2 = union, 3 = sym_difference
// ============================================================================
fn serial_set_operation_count(
    a_begin: u32,
    a_end: u32,
    b_begin: u32,
    b_end: u32,
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        ...
        var do_emit: bool;
        if (OP_MODE == 0u) { do_emit = (pA == pB); }
        else if (OP_MODE == 1u) { do_emit = pA; }
        else if (OP_MODE == 2u) { do_emit = true; }
        else { do_emit = (pA != pB); }
        if (do_emit) { count += 1u; }
...
fn decoupled_lookback_batched_kernel(
    ...
    // ========================================================================
    // Step 0: Load pair info from wgInfo (thread 0)
    // ========================================================================
    if (tid == 0u) {
        let infoBase = block * WG_INFO_STRIDE;
        wg_pairAStart     = wgInfo[infoBase + 0u];
        wg_pairALen       = wgInfo[infoBase + 1u];
        wg_pairBStart     = wgInfo[infoBase + 2u];
        wg_pairBLen       = wgInfo[infoBase + 3u];
        wg_localWgIdx     = wgInfo[infoBase + 4u];
        wg_pairNumWg      = wgInfo[infoBase + 5u];
        wg_globalWgOffset = wgInfo[infoBase + 6u];
        wg_dpiOffset      = wgInfo[infoBase + 7u];
    }
    workgroupBarrier();
    ...
    // ========================================================================
    // Step 4: Serial intersection count (no output write)
    // ========================================================================
    let local_count = serial_set_operation_count(
        a0tid, a_count2,
    ...
    // No Step 7 (scatter) needed - we only count, don't write elements
}
