/**
 * GPU-accelerated Anti-Join via Set Difference By Key
 *
 * Implements: SELECT * FROM A WHERE A.key NOT IN (SELECT key FROM B)
 * Returns rows from A whose key does NOT exist in B.
 *
 * Uses the 2-Phase by_key pipeline with OP_MODE=1 (difference):
 *   1. DPI — compute merge path boundaries on keys
 *   2. Decoupled Lookback By Key (OP_MODE=1) — output A rows where A.key < B.key
 */

import TimestampQueryManager from '../../TimestampQueryManager';
import { setOpMode, setDifferenceCPUByKey } from '../../utils';
...
class AntiJoin {
    ...
    constructor(device: GPUDevice, tsm: TimestampQueryManager) {
        ...
        // OP_MODE=1: difference (A \ B)
        this.lookbackPipeline = device.createComputePipeline({
            label: 'AntiJoin Lookback pipeline',
            layout: device.createPipelineLayout({ bindGroupLayouts: [this.lookbackBindGroupLayout] }),
            compute: { module: device.createShaderModule({ code: setOpMode(lookbackByKeyShaderBase, 1) }), entryPoint: 'decoupled_lookback_by_key_kernel' }
        });
    }
    ...
    async run(aKeys: Uint32Array, aValues: Uint32Array, bKeys: Uint32Array, iterations = 1, warmup = 0) {
        ...
        if (total === 0) return { totalCount: 0, timing: { dpiMs: 0, lookbackMs: 0, totalMs: 0 } };

        const numWg = Math.ceil(total / NV);
        const maxOutputSize = a_len;  // difference: all of A could survive
        ...
        return { totalCount, timing: { dpiMs: avg(dpiTimes), lookbackMs: avg(lbTimes), totalMs: avg(totalTimes) } };
    }
}

export async function runAntiJoinTest(device: GPUDevice): Promise<void> {
    console.log('\n========================================================================');
    console.log('  ANTI-JOIN via GPU Set Difference By Key');
    console.log('  SELECT * FROM A WHERE A.key NOT IN (SELECT key FROM B)');
    console.log('  DPI (keys only) + Decoupled Lookback By Key (OP_MODE=1)');
    console.log('========================================================================\n');
    ...
    const joiner = new AntiJoin(device, tsm);

    console.log(`  ${NUM_WARMUP} warmup + ${NUM_ITERATIONS} iterations\n`);
    console.log('  Dataset                | |A|          | |B|          | Anti-Join    | DPI(ms)   | Lookback(ms) | Total(ms) | Match');
    console.log('  -----------------------|--------------|--------------|--------------|-----------|--------------|-----------|------');
    ...
    for (const ds of tpchDatasets) {
        try {
            ...
            // CPU validation using setDifferenceCPUByKey
            let matchStr = 'skip';
            if (aKeys.length <= 64_000_000) {
                ...
                // B needs interleaved format too for setDifferenceCPUByKey
                const bInterleaved = new Uint32Array(bKeys.length * 2);
                for (let i = 0; i < bKeys.length; i++) { bInterleaved[i * 2] = bKeys[i]; bInterleaved[i * 2 + 1] = 0; }
                const cpuResult = setDifferenceCPUByKey(aInterleaved, bInterleaved);
