...
export function setSymmetricDifferenceCPU(setA: Uint32Array, setB: Uint32Array): Uint32Array {
    ...
    while (i < setA.length && j < setB.length) {
        ...
        if (a < b) {
            ...
            while (i < setA.length && setA[i] === key) {
                i++;
            }
            while (j < setB.length && setB[j] === key) {
                j++;
            }

            const countA = i - startA;
            const countB = j - startB;

            if (countA > countB) {
                for (let k = startA + countB; k < i; k++) {
                    result.push(setA[k]);
                }
            } else if (countB > countA) {
                for (let k = startB + countA; k < j; k++) {
                    result.push(setB[k]);
                }
            }
        }
