...
export class GPUSorter {
    ...
    public createKeyvalBuffers(length: number): [GPUBuffer, GPUBuffer, GPUBuffer, GPUBuffer] {
        ...
        // console.log("key buffer size: ", this.keysBufferSize(length));
        // console.log("add padding: ", count_ru_histo);
    ...
    public createSortBuffers(length: number): SortBuffers {
        ...
        // console.log("num_keys (length): ", uniform_infos.num_keys);
    ...
    public recordCalculateHistogram(commandEncoder: GPUCommandEncoder, bindGroup: GPUBindGroup, length: number, timestampManager?: TimestampQueryManager, baseIndex: number =0) {
        // as we only deal with 32 bit float values always 4 passes are conducted
        const histBlocksRu = this.histoBlocksRu(length);

        const passEncoder = commandEncoder.beginComputePass(timestampManager? {
            label: "zeroing histogram",
            ...timestampManager.createComputePassDescriptor(
            baseIndex + 0,
            baseIndex + 1
            ),} : { label: "zeroing histogram" });
        ...
        const passEncoder2 = commandEncoder.beginComputePass(timestampManager
          ? {
    ...
    public recordPrefixHistogram(commandEncoder: GPUCommandEncoder, bindGroup: GPUBindGroup, timestampManager?: TimestampQueryManager, baseIndex: number = 0) {
        const passEncoder = commandEncoder.beginComputePass(timestampManager
          ? {
    ...
    public recordScatterKeys(commandEncoder: GPUCommandEncoder, bindGroup: GPUBindGroup, length: number, timestampManager?: TimestampQueryManager, baseIndex: number = 0) {
    ...
    public async sort(queue: GPUQueue, sortBuffers: SortBuffers, sortFirstN?: number, iterationIndex: number = 0) {
        ...
        const base = iterationIndex * QUERIES_PER_ITER;
        this.recordCalculateHistogram(commandEncoder, sortBuffers.bindGroup, numElements, this.timestampQueryManager, base + 4);
        this.recordPrefixHistogram(commandEncoder, sortBuffers.bindGroup, this.timestampQueryManager, base + 8);
        this.recordScatterKeys(commandEncoder, sortBuffers.bindGroup, numElements, this.timestampQueryManager, base + 10);
