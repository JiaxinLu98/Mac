...
@group(0) @binding(0) var<storage, read_write> a_in_hash_table: array<Entity>;
...
@group(0) @binding(4) var<storage, read_write> result: array<vec2<u32>>;
...
fn get_join_result(@builtin(global_invocation_id) global_id: vec3<u32>, 
    ...
    for(var i = begin; i < end; i = i + 1u) {
        ...
        var probe_count = 0u;
        loop {
            let current_key = atomicLoad(&a_in_hash_table[position].key);
            if(key == current_key) {
                let newT_index = atomicAdd(&result_size, 1u);
                result[newT_index].x = key;
                result[newT_index].y = a_in_hash_table[position].value;
                break;
            }
