...
export class GPUSetIntersectionByKey {
    ...
    public async computeIntersection(
        ...
        if (setA.length % 2 !== 0) {
            throw new Error('setA must contain (key,value) pairs (length must be even).');
        }
        ...
        for(let i = 0; i < iters; i++) {
            ...
            // API CALL: MARK
            const recordedFlags = await this.runMarkPhase(bufferA, bufferB, bufferFlags, setA.length/2, setB.length);
            // API CALL: SCAN
    ...
    private async runMarkPhase(
        ...
        commandEncoder.copyBufferToBuffer(flagsBuffer, bufferRecordFlags, lenA * Uint32Array.BYTES_PER_ELEMENT);
