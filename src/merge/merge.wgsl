...
fn cmp_gt(a: u32, b: u32) -> bool {
    return a > b;
}

fn cmp_lte(a: u32, b: u32) -> bool {
    return a <= b;
}
...
fn compute_diagonals(@builtin(global_invocation_id) global_id: vec3<u32>,
    ...
    // Ensure only one thread in the workgroup executes this code
...
const K = 512u; // Merge window size. Must be 4 * workgroup_size_x
const POSITIVE_INFINITY = 0xFFFFFFFFu;
const NEGATIVE_INFINITY = 0u;
const block_dim_x = 128; // workgroup_size_x

// Shared (workgroup) memory for the local merge window
var<workgroup> elements: array<u32, (K+2u) << 1 >; // K+2 elements for sentinels
const A_shared = 0;
const B_shared = K + 2u;
...
fn merge_single_path(@builtin(local_invocation_id) local_id: vec3<u32>,
    ...
    var binary_search_count: u32 = 0u;
