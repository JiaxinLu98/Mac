// ============================================================================
// Balanced Path with Biased Binary Search - BATCHED VERSION, COMPACT METADATA
//
// Same algorithm as balanced_path_batched.wgsl. Pair-level metadata is stored
// once per pair (pairInfo) instead of once per workgroup; each workgroup only
// stores its pair id. keysA/keysB are the tidsets' own buffers (read in place).
//
// pairInfo layout per pair (8 x u32, stride = PAIR_INFO_STRIDE):
...
//   [4] pairNumWg    - total workgroups for this pair
//   [5] stateOffset  - first workgroup of this pair (localWgIdx = wg - stateOffset)
//   [6] dpiOffset    - start of this pair's region in DPI buffer
//   [7] outputOffset - start of this pair's region in the output buffer
// ============================================================================
...
@group(0) @binding(3) var<storage, read> pairInfo: array<u32>;
@group(0) @binding(4) var<storage, read> pairIdPerWg: array<u32>;
@group(0) @binding(5) var<uniform> totalWg_uniform: u32;

const STAR_MASK: u32 = 0x80000000u;
const WORKGROUP_SIZE: u32 = 256u;
const PAIR_INFO_STRIDE: u32 = 8u;
...
fn compute_diagonals_batched(
    ...
    // Read pairInfo for targetWg's pair (all lanes read the same pair info)
    // Clamp to valid range for subgroup uniformity
    let safeWg = min(targetWg, max(totalWg, 1u) - 1u);
    let infoBase = pairIdPerWg[safeWg] * PAIR_INFO_STRIDE;
    let pairAStart = pairInfo[infoBase + 0u];
    let pairALen   = pairInfo[infoBase + 1u];
    let pairBStart = pairInfo[infoBase + 2u];
    let pairBLen   = pairInfo[infoBase + 3u];
    let pairNumWg  = pairInfo[infoBase + 4u];
    let localWgIdx = safeWg - pairInfo[infoBase + 5u];
    let dpiOffset  = pairInfo[infoBase + 6u];
